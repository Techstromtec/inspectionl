 ...
 <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-validate/1.19.3/jquery.validate.min.js"></script>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Capture the selected area when the area dropdown value changes
            $('#area').on('change', function() {
                var selectedAreaId = $(this).val();

                // Send the selected area's ID to the backend using an AJAX request
                $.ajax({
                    url: '/get-pincode/'+selectedAreaId, // Replace with your backend route
                    method: 'GET',
                    success: function(data) {
                        // Fill the pin code input field with the retrieved pin code
                        $('#pincode').val(data.pincode[0].pincode);
                    },
                    error: function(xhr, textStatus, errorThrown) {
                        // Handle any errors here
                        console.error(errorThrown);
